...
export class MkCentralTournamentsApi {
  constructor() {
    ...
    this.imgBaseUrl = 'https://mkcentral.com/img';
  ...
  transformTournament(raw) {
    ...
    // Build logo URL - prioritize tournament logo, then series logo
    let logo = null;
    if (raw.logo_filename) {
      // Check if it's a direct filename or a path
      if (raw.logo_filename.includes('/')) {
        logo = `https://www.mariokartcentral.com/mkc/img/mkcv1_images/${raw.logo_filename}`;
      } else {
        logo = `${this.imgBaseUrl}/mkcv1_images/event_logos/${raw.logo_filename}`;
      }
    }
    // Try tournament-specific logo on mkcentral.com
    const tournamentLogo = `${this.imgBaseUrl}/tournament_logos/${raw.event_id}.png`;
    
    // Series logo as fallback
    let seriesLogo = null;
    if (raw.series_logo_filename) {
      seriesLogo = `https://www.mariokartcentral.com/mkc/img/mkcv1_images/${raw.series_logo_filename}`;
    } else if (raw.tournament_series_id) {
      seriesLogo = `${this.imgBaseUrl}/series_logos/${raw.tournament_series_id}.png`;
    }
    ...
    return {
      ...
      // Media - use tournament logo first, then series logo
